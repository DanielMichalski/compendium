...
/**
 * Program demonstruj¹cy dzia³anie mechanizmu "przeci¹gnij i upuœæ"
 * na przyk³adzie listy obrazków.
      ...
      try
      {
         if (flavors.contains(DataFlavor.javaFileListFlavor))
         {
            ...
            for (File f : fileList)
            {
               ...
               {
                  // obrazek nie zosta³ odczytany - pomija
               }
            }
         }
         ...
         if (support.isDrop())
         {
            ...
            if (!location.isInsert()) model.remove(index); // miejsce zast¹pienia
         }
